...
export class SubcategoryComponent implements OnInit {
  ...
  constructor(private api: ApiService, private route: ActivatedRoute,private router:Router, private sanitizer: DomSanitizer) { 
    ...
    this.route.params.subscribe(
      params=>{
          ...
          this.id = this.catall.find(x => x.category.toLowerCase() == params.subcategory);
  ...
  go(value){
    if(this.mid){
      this.router.navigate(['manufacture', this.mname,this.name.toLowerCase() ,value.replace(/ /g, "-")],{queryParams:{'manufacture':this.mid}});
    }
    else{
      this.router.navigate(['/', this.name.toLowerCase() ,value.replace(/ /g, "-")]);
    }
  }
